...
class RPCRequest {
    ...
    execute(endpoint, params) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            http.send(uri);
            Util.info("Fetching endpoint %s".format_string(url))
        });
    }
...
class ModelRequest {
    ...
    constructor(jsonrpc, endpoint, uuid, token, model) {
    ...
    read(id, fields = []) {
        return this.call('read', [id, {fields: fields}]);
    }
    ...
    search_count(filter = []) {
        return this.call('search_count', [filter]);
    }
    ...
    unlink(id) {
        return this.call('unlink', [id]);
    }

    call(method, args) {
        let credentials = [this.uuid, this.token, this.model, method];
        let params = credentials.concat(args);
        return this.jsonrpc.execute(this.endpoint, 'execute_kw', params);
    }
...
class TreeRequest {
    ...
    constructor(content) {
        this.jsonrpc = new JsonRPC(content.host);
    ...
    env(model) {
        return new ModelRequest(this.jsonrpc, this.endpoint, this.uuid, this.token, model);
    }
    ...
    fetch() {
        ...
        let self = this;
        self.env(self.model)
            .search_count(self.filter)
            .then(function (count) {
                self.count = count;
                self.env(self.model)
                    .search_read(self.filter, self.fields, self.limit, self.offset, self.sort)
                    .then(function (data) {
                        self.onPageFetched(data);
                    }).catch(function (e) {
                    console.error(e);
                });
            }).catch(function (e) {
...
class ApiManager {
    ...
    execute(host, endpoint, method, queryParams, formData, withBody=false) {
        ...
        if (!Util.isEmptyDictionary(queryParams))
            url = url + '?' + Util.dictToURI(queryParams);
        ...
        return new Promise(function (resolve, reject) {
            ...
            if (formData){
                if(!withBody)
                    http.send(Util.dictToURI(formData));
                else
            ...
            //Util.info("%s : %s".format_string(method, url))
        });
    }
}
...
class ChatConsumer {
    ...
    initializeWebsocket(uuid,token) {
        ...
        let loc = new URL(this.server_host);
    ...
    begin() {
        ...
        req.get('/bigbot/server').then(function (server){
            self.server_host = server.server_host;
            self.jsonrpc = new JsonRPC(self.server_host);

            //main stream
            if(self.cm.hasCookie('uuid') && self.cm.hasCookie('token')){
                const uuid = self.cm.getCookie('uuid');
                const token = self.cm.getCookie('token');
                Util.info("Cache token already present.");
                Util.info('Checking if exiting token is valid or not...')
                const params = [uuid,token];
                self.jsonrpc.execute('/jsonrpc/consumer', 'authenticate' , params).then(function (data) {
                    Util.info("Exiting user token is valid.");
                    self.initChat(uuid,token);
                }).catch(function (status) {
                    Util.info("Token Present But Not Valid");
                    Util.info("Removing old token...");
                    self.cm.removeCookie('uuid');
                    self.cm.removeCookie('token');
                    Util.info("Generating New Token....");
                    self.getToken();
                });
            }else {
                Util.info("Token Not Present. Generating New....");
                self.getToken();
            }
        });
    }
    ...
    getToken() {
        const self = this;
        const params = [];
        this.api.get(this.client_host,'/bb/token', params).then(function (data) {
            Util.info("New Token Generated successfully !!!");
            const result = JSON.parse(data).result;
            self.cm.setCookie('uuid', result.uuid);
            self.cm.setCookie('token', result.token);
            Util.info("Token saved to cookies.");
            self.initChat(result.uuid, result.token);
        }).catch(function (e) {
            Util.error('Unable to generate token from client server.');
        });
    }
    ...
    initChat(uuid, token){
        ...
        self.jsonrpc.execute('/jsonrpc/consumer', 'get_style' , params).then(function (stylesheet) {
            ...
            self.jsonrpc.execute('/jsonrpc/consumer', 'get_active_channel' , params).then(function (active_channel) {
                Util.info("Active channel info received.");
                ...
                self.jsonrpc.execute('/jsonrpc/consumer', 'get_messages' , params).then(function (history) {
                    Util.info("Active channel history received.");
    ...
    getChannels(uuid, token){
        ...
        self.jsonrpc.execute('/jsonrpc/consumer', 'get_channels' , params).then(function (channels) {
            Util.info("All channels information received.");
    ...
    getChatHistory(channel) {
        Util.info('Looking for channel history');
        const self = this;
        const uuid = self.cm.getCookie('uuid');
        const token = self.cm.getCookie('token');
        const params = [uuid,token, channel.channel_uuid];
        self.jsonrpc.execute('/jsonrpc/consumer', 'get_messages' , params).then(function (history) {
            Util.info("Selected Channel history received.");
            self.history = history;
            self.onMessagesReceived(history);
        }).catch(function (e) {
        });
    ...
    onInputTextChanged(query = "") {
        Util.info('Looking for suggestion...');
        const self = this;
        const uuid = self.cm.getCookie('uuid');
        const token = self.cm.getCookie('token');
        ...
        self.jsonrpc.execute('/jsonrpc/consumer', 'get_suggestions' , params).then(function (suggestions) {
            Util.info("New Suggestions received.");
