...
class RasaFileGenerator:
    ...
    def generate_rules(self):
        ...
        for intent in self.payload['intents']:
            if "form" in intent:
                ...
                rules.append(submit)
            elif("chitchat/" in intent["intent"]):
                value=intent["intent"].split("/")
                if chitchat_count==0:
                    rule = {
                    "rule": f"{intent['intent']}",
                    "steps": [
                        {"intent": value[0]},
                        {"action": f"utter_{value[0]}"}
    ...
    def generate_domain_file(self):
        ...
        yaml_output = "version: '3.1'\n"
        yaml_output += "intents:\n"
        chitchat_intent_count=0
        for intent in intents:
            if "form" in intent:
                yaml_output += f"  - {intent['intent']}\n"
            elif "chitchat/" in intent["intent"]:
                val=intent["intent"].split("/")
                if chitchat_intent_count==0:
                    yaml_output+=f"  - {val[0]}\n"
                    chitchat_intent_count=1
                else:continue
            else:
                yaml_output += f"  - {intent['intent']}\n"
