...
class AgentNluGenerator:
    ...
    def create_rasa_folder_structure(self):
        skill_path = f"./agent/{self.skill_id}"
    ...
    def nlu_yml(self):
            nlu_file = os.path.join("agent",self.skill_id, "data", "nlu.yml")
    ...
    def upload_config_file(self, config_file_path):
        shutil.copyfile(config_file_path, os.path.join("agent",self.skill_id, 'config.yml'))
        return "Config file uploaded successfully"

    def train_and_save_model1(self):
        print("yess")

        config = f"agent/{self.skill_id}/config.yml"
        training_files = f"agent/{self.skill_id}/data/"
        output = f"agent/{self.skill_id}/models/"

        rasa.train(config, [training_files], output, fixed_model_name='rasa_model')

    def train_and_save_model(self):
        command = f"rasa train nlu --config agent/{self.skill_id}/config.yml --nlu agent/{self.skill_id}/data/nlu.yml --out agent/{self.skill_id}/models"
