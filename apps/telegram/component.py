...
@authenticate
def on_callback(update, context, *, profile):
...
@authenticate
def on_channel_switch(update, context, *, profile):
...
@authenticate
def on_help(update, context, *, profile):
    context.bot.send_message(
        chat_id=update.effective_chat.id,
        text=""
...
@authenticate
def on_message(update, context, *, profile):
...
def on_start(update, context):
    ...
    context.bot.send_message(chat_id=update.effective_chat.id, text="Bridge created successfuly")


@authenticate
def on_unlink(update, context, *, profile):
    profile.delete()
...
class TelegramBot(ChatPlatform):
    ...
    def init(self):
        handlers = [
            ...
            CommandHandler("switch", on_channel_switch),
            CommandHandler("unlink", on_unlink),
    ...
    def on_message(self, profile, message):
